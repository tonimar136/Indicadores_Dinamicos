                ...
                <div class="card-body">
                    ...
                    <form action="controller/indicadores-controller.php" method="POST">
                        <div class="form-group">
                            ...
                            <input type="hidden" name="controle" value="<?=$ctrl?>">
                            <?php
                                $j = 0;
                                $t = 1;
                                            ...
                                            <?php
                                                ...
                                                if($formulario[$j]['tipo_resposta'] == 'SELECT *'){
                                                    echo '<select class="form-control" name="resposta'.$t.'">';
                                                        ...
                                                        $conta = count($select);
                                                        while($g < $conta){
                                                            $info = $ind->consRespEsp($select[$g]['id'], $ctrl);
                                                            $c = count($info);
                                                            if($c == 1){
                                                                echo '<option value="'.$select[$g]['id'].'" selected>'.$select[$g]['descricao'].'</option>';
                                                            }else{
                                                    ...
                                                    while($g < $conta){
                                                        ...
                                                        $c = count($info);
                                                        if($c == 1){
                                                            echo '<br><input type="radio" name="resposta'.$t.'" value="'.$select[$g]['id'].'" checked>'.$select[$g]['descricao'];
                                                        }else{
                                                    ...
                                                    while($g < $conta){
                                                        ...
                                                        $c = count($info);
                                                        if($c == '1'){
                                                            echo '<br><input type="checkbox" name="resposta-ck'.$t.'-'.$g.'" value="'.$select[$g]['id'].'" checked>'.$select[$g]['descricao'];
                                                        }else{
                                                    ...
                                                    $info = $ind->consRespEsp1($formulario[$j]['id_pergunta'], $ctrl);
                                                    if($formulario[$j]['tipo_resposta'] == 'DATA'){
                                                        echo '<input type="date" class="form-control" name="resposta'.$t.'" value="'.$info[0]['resposta'].'"';
                                                    }elseif($formulario[$j]['tipo_resposta'] == 'APENAS NÚMEROS'){
                                                        echo '<input type="text" class="form-control apenas-numeros" name="resposta'.$t.'" value="'.$info[0]['resposta'].'">';
                                                    }elseif($formulario[$j]['tipo_resposta'] == 'CEP'){
                                                        echo '<input type="text" class="form-control cep-input" name="resposta'.$t.'" maxlength="9" value="'.$info[0]['resposta'].'">';
                                                    }elseif($formulario[$j]['tipo_resposta'] == 'CPF/CNPJ'){
                                                        echo '<input type="text" class="form-control cpf-cnpj" name="resposta'.$t.'" value="'.$info[0]['resposta'].'"';
                                                    }elseif($formulario[$j]['tipo_resposta'] == 'TEXTO ÁREA'){
                                                        echo '<textarea class="form-control" name="resposta'.$t.'" rows="3">'.$info[0]['resposta'].'</textarea>';
                                                    }else{
                                                        echo '<input type="text" class="form-control" name="resposta'.$t.'" value="'.$info[0]['resposta'].'">';
                                                    }
                                                }
                                            ?>
                                        </div>
                        ...
                        <input style="float: right;" type="submit" class="btn btn-primary" name="atualizarIndicadores" value="Atualizar">
                    </form>
                </div>          
            </div>
        </div>
...
<?php
    if($_GET['url'] == 'indicadores-form-edit-ins'){
        echo '
            <script>
                Swal.fire({
                    position: "top-end",
    ...
    if($_GET['url'] == 'indicadores-form-edit-fail'){
        echo '
            <script>
                Swal.fire({
                    position: "top-end",
